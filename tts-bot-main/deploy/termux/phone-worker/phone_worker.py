...
def _core_worker_source() -> str:
    default = "windows-pc-worker" if os.name == "nt" else "termux-phone-worker"
    return _short_text(os.getenv("CORE_WORKER_SOURCE") or os.getenv("PHONE_WORKER_SOURCE") or default, limit=32, default=default)


def _core_worker_runtime_mode() -> str:
    raw = str(os.getenv("CORE_WORKER_RUNTIME_MODE") or os.getenv("PHONE_WORKER_RUNTIME_MODE") or "").strip().lower().replace("_", "-")
    if raw:
        return _short_text(raw, limit=32, default="termux")
    if os.name == "nt":
        return "windows-pc"
    source = _core_worker_source().lower().replace("_", "-")
    if "pc" in source and "termux" not in source:
        return "linux-pc"
    return CORE_WORKER_RUNTIME_MODE


def _core_worker_device_type() -> str:
    raw = str(os.getenv("CORE_WORKER_DEVICE_TYPE") or os.getenv("CORE_WORKER_KIND") or os.getenv("PHONE_WORKER_DEVICE_TYPE") or "").strip().lower().replace("-", "_")
    if raw:
        return _short_text(raw, limit=32, default="android_termux")
    if os.name == "nt":
        return "windows_pc"
    source = _core_worker_source().lower().replace("_", "-")
    mode = _core_worker_runtime_mode().lower().replace("_", "-")
    if "pc" in source or "pc" in mode:
        return "linux_pc"
...
def _core_worker_payload(*, host: str, port: int) -> dict[str, Any]:
    ...
    runtime_mode = _core_worker_runtime_mode()
    source = _core_worker_source()
    device_type = _core_worker_device_type()
    return {
        "worker_id": worker_id,
        "name": _short_text(name, limit=64, default="Core Worker"),
        "source": source,
        ...
        "status": {
            ...
            "runtime": {
                ...
                "migration_stage": "core-worker-universal",
                "summary": "Worker universal executa jobs; Android/Termux e PC usam o mesmo registry.",
            },
            "profile": profile,
            "profile_label": _core_worker_profile_label(profile),
            "http_host": host,
...
def _pair_core_worker(
    *,
    ...
    payload.update({
        ...
        "name": _short_text(selected_name, limit=64, default="Core Worker"),
        "source": _core_worker_source(),
        "device_type": _core_worker_device_type(),
    })
    ...
    env_path = _update_env_file(env_file, {
        ...
        "CORE_WORKER_SOURCE": payload.get("source") or _core_worker_source(),
        "CORE_WORKER_DEVICE_TYPE": payload.get("device_type") or _core_worker_device_type(),
        "CORE_WORKER_RUNTIME_MODE": _core_worker_runtime_mode(),
...
def _legacy_audio_node_removed_status(reason: str = "health") -> dict[str, Any]:
    return {
        "attempted": False,
        "removed": True,
        "reason": "legacy_audio_node_removed",
        "mode": "direct_music_agent",
    }
...
def _music_node_snapshot() -> dict[str, Any]:
    return {
        ...
        "reason": "audio_node_legacy_removed",
    }
...
def _music_agent_snapshot() -> dict[str, Any]:
    ...
    try:
        ...
        # YouTube direto usa voz/ffmpeg do Music Agent. Não há node de áudio
        # separado: playlist/Spotify/SoundCloud entram como metadados e resolvem
        # stream direto perto da reprodução.
...
def _system_status() -> dict[str, Any]:
    ...
    return {
        ...
        "source": _core_worker_source(),
        "runtime": {
            "mode": _core_worker_runtime_mode(),
            "current_worker": _core_worker_source(),
            "device_type": _core_worker_device_type(),
            "internal_runtime": CORE_WORKER_INTERNAL_RUNTIME_STATE,
            "migration_stage": "core-worker-universal",
        },
...
def _local_agent_status_payload(*, host: str, port: int) -> dict[str, Any]:
    ...
    return {
        ...
        "note": "Rota local do Core Worker; Android/Termux e PC usam o mesmo registry sem expor token.",
    }
...
class WorkerHandler(BaseHTTPRequestHandler):
    ...
    def _task_log_summary(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        patterns = {
            ...
            "phone_worker": r"phone-worker|core-worker",
        }
...
def main() -> int:
    _load_env_file()
    _load_persisted_pending_core_job_results()
    parser = argparse.ArgumentParser(description="Core Worker universal para tarefas opcionais da VPS.")
