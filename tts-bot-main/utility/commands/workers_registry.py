...
def _compact_worker_public(record: Mapping[str, Any], *, now: float | None = None) -> dict[str, Any]:
    ...
    device_type = _short_text(
        record.get("device_type")
        or status.get("device_type")
        or runtime.get("device_type")
        or record.get("platform")
        or status.get("platform")
        or "",
        limit=32,
    )
    if not device_type:
        hint = " ".join(str(x or "") for x in (record.get("source"), runtime_mode, runtime.get("current_worker"))).lower().replace("_", "-")
        if "windows" in hint:
            device_type = "windows_pc"
        elif "linux-pc" in hint or "pc-worker" in hint:
            device_type = "linux_pc"
        elif "android" in hint or "apk" in hint:
            device_type = "android"
        elif "termux" in hint or "phone" in hint:
            device_type = "android_termux"
    public = {
...
class CoreWorkersRegistry:
    ...
    def redeem_pairing(self, payload: Mapping[str, Any], *, remote_addr: str = "") -> dict[str, Any]:
        ...
        with self._lock:
            ...
            device_type = _short_text(payload.get("device_type") or payload.get("platform") or payload.get("kind"), limit=32)
    ...
    def ensure_direct_worker(self, payload: Mapping[str, Any], *, token: str, remote_addr: str = "") -> dict[str, Any]:
        ...
        with self._lock:
            ...
            record.update({
                ...
                "device_type": _short_text(payload.get("device_type") or payload.get("platform") or record.get("device_type") or "android_termux", limit=32),
                "name": _short_text(payload.get("name") or record.get("name") or "Core Worker", limit=64),
            })
    ...
    def heartbeat(self, payload: Mapping[str, Any], *, token: str, remote_addr: str = "") -> dict[str, Any]:
        ...
        with self._lock:
            ...
            for key in ("name", "endpoint", "version", "source", "device_type"):
                if key in payload:
                    record[key] = _short_text(payload.get(key), limit=160 if key == "endpoint" else 64)
            if "platform" in payload and "device_type" not in payload:
                record["device_type"] = _short_text(payload.get("platform"), limit=32)
            if "roles" in payload:
