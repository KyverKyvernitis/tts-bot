...
WORKER_ROLE_PROFILE_DESCRIPTIONS: dict[str, str] = {
    ...
    "builder": "compilar APK no Core Worker",
    "turbo": "máximo desempenho para acelerar a VPS",
    "bedrock": "Minecraft Bedrock futuro",
}

WORKER_EDITABLE_FEATURES: tuple[dict[str, Any], ...] = (
    {"value": "phone-worker", "label": "Base", "description": "Obrigatório para qualquer worker", "emoji": "📱", "roles": ("phone-worker",), "capabilities": ("phone-worker",), "tasks": ("ping", "status")},
...
WORKER_ACTION_SPECS: tuple[dict[str, Any], ...] = (
    ...
    {"label": "Atualizar agent", "value": "worker_update_quick", "job_type": "worker_update", "payload": {}, "summary": "atualizar arquivos do Core Worker", "description": "Corrige agent antigo", "emoji": "⬆️", "category": "quick"},
    ...
    {"label": "Atualizar agent", "value": "worker_update", "job_type": "worker_update", "payload": {}, "summary": "atualizar arquivos do Core Worker", "description": "Atualiza e reinicia", "emoji": "⬆️", "requires_declared": True, "category": "maintenance"},
    ...
    {"label": "Testar VPS pelo worker", "value": "endpoint_probe", "job_type": "endpoint_probe", "payload": {"targets": ["auto"]}, "summary": "testar endpoints da VPS a partir do worker", "description": "Ping HTTP real", "emoji": "📡", "requires_declared": True, "category": "assist"},
    ...
    {"label": "Logs", "value": "worker_logs", "job_type": "worker_logs", "payload": {"lines": 140}, "summary": "logs recentes do Core Worker", "description": "Mostra logs recentes", "emoji": "📜", "category": "quick"},
    ...
    {"label": "Status serviços", "value": "service_status", "job_type": "service_status", "payload": {"service": "phone-worker"}, "summary": "status de serviços do worker", "description": "Serviços permitidos", "emoji": "🧰", "category": "monitor"},
    ...
    {"label": "Reiniciar worker", "value": "service_restart_worker", "job_type": "service_restart", "payload": {"service": "phone-worker"}, "summary": "reiniciar Core Worker", "description": "Reinicia agent", "emoji": "🔁", "category": "maintenance"},
    {"label": "Parar worker", "value": "service_stop_worker", "job_type": "service_stop", "payload": {"service": "phone-worker"}, "summary": "parar Core Worker", "description": "Para agent", "emoji": "🛑", "category": "maintenance"},
)
...
WORKER_ACTION_CATEGORIES: tuple[dict[str, str], ...] = (
    ...
    {"label": "Adicionar worker", "value": "add", "description": "Pareamento e guia simples", "emoji": "📲"},
)
...
def _worker_device_label(worker: dict[str, Any] | None) -> str:
    if not isinstance(worker, dict):
        return ""
    status = worker.get("status") if isinstance(worker.get("status"), dict) else {}
    ...
    raw_values = [
        worker.get("device_type"),
        status.get("device_type"),
        worker.get("runtime_mode"),
        status.get("runtime_mode"),
        runtime.get("mode"),
        runtime.get("current_worker"),
        worker.get("source"),
    ]
    raw = " ".join(str(value or "") for value in raw_values).strip().lower().replace("_", "-")
    if "windows" in raw:
        return "Windows PC"
    if "linux-pc" in raw or "pc-worker" in raw or "desktop" in raw:
        return "Linux PC"
    if "android" in raw or "apk" in raw:
        return "Android"
    if "termux" in raw or "phone" in raw:
...
def _core_worker_app_runtime_text(worker_id: str) -> str:
    ...
    "Detalhes do worker". Aqui evitamos despejar estado técnico/truncado.
    """
    worker_id = str(worker_id or "").strip()
    if not worker_id:
...
def _core_worker_push_status_text(worker_id: str = "") -> str:
    ...
    if status == "failed":
        return "Push: falhou no último envio · detalhes no worker"
...
def _watch_output_note(output: object) -> str:
    ...
    if "phone_worker_ssh_user vazio" in lowered:
        return "SSH do worker não configurado; aguardando watchdog local"
    ...
    if "probe ssh:" in lowered and "connection-refused" in lowered:
        return "porta SSH fechada/sshd parado no worker"
    ...
    if "probe ssh:" in lowered and "no-route" in lowered:
        return "sem rota da VPS até o worker"
    ...
    if "probe worker-http:" in lowered and "connection-refused" in lowered:
        return "porta do phone-worker fechada no worker"
    ...
    if "não consegui acionar" in lowered or "nao consegui acionar" in lowered:
        ...
        return "SSH falhou ou worker não respondeu"
    if "worker respondeu ao ssh" in lowered:
...
def _worker_detail_text(worker: dict[str, Any] | None) -> str:
    if not isinstance(worker, dict) or not worker:
        return "Selecione um worker registrado para ver os detalhes."
...
class WorkerRolesEditorView(discord.ui.LayoutView):
    ...
    def _rebuild(self, *, done: str = "") -> None:
        ...
        features_select = self._new_select(
            placeholder="Funções deste worker",
            min_values=1,
...
class WorkersPanelView(discord.ui.LayoutView):
    ...
    def _has_legacy_worker(self) -> bool:
        # O Core Worker direto é só fallback. Quando há worker registrado online,
    ...
    def _worker_select_options(self) -> list[discord.SelectOption]:
        ...
        if self._has_legacy_worker():
            ...
            options.append(discord.SelectOption(
                label=_shorten(self.snapshot.name or "Core Worker direto", limit=100),
    ...
    def _selected_supported_tasks(self) -> set[str] | None:
        ...
        if self._selected_is_legacy():
            ...
            # O Core Worker direto continua sendo o builder/agent mesmo sem worker
            # pareado pelo APK. Manter ações essenciais visíveis evita regressão
            # quando o registry ainda mostra "nenhum worker pareado".
            legacy_direct = {
    ...
    def _panel_action_specs_for_selected(self) -> list[dict[str, Any]]:
        ...
        specs: list[dict[str, Any]] = [
            {"label": "Gerar código", "value": "_pairing_modal", "description": "Parear novo worker", "emoji": "🔐", "panel_action": "pair", "category": "add"},
        ...
        if selected_worker is not None:
            specs.extend([
                {"label": "Detalhes do worker", "value": "_worker_details", "description": "Resumo completo", "emoji": "📱", "panel_action": "details", "category": "quick"},
                ...
                {"label": "Renomear worker", "value": "_rename_worker", "description": "Troca o nome exibido", "emoji": "✏️", "panel_action": "rename", "category": "organize"},
            ...
            if selected_worker.get("enabled", True):
                specs.append({"label": "Pausar worker", "value": "_pause_worker", "description": "Não recebe jobs", "emoji": "⏸️", "panel_action": "pause", "category": "organize"})
            else:
                specs.append({"label": "Ativar worker", "value": "_resume_worker", "description": "Volta a receber jobs", "emoji": "▶️", "panel_action": "resume", "category": "organize"})
    ...
    def _action_select_options(self) -> list[discord.SelectOption]:
        ...
        if not specs:
            return [discord.SelectOption(
                ...
                value="_unsupported",
                description="Atualize o worker ou escolha outro worker",
                emoji="⚠️",
            )]
        return [
    ...
    def _rebuild_layout(self, *, expired: bool = False) -> None:
        ...
        if registered <= 0:
            workers_label = "worker direto online" if snapshot.online else "nenhum worker pareado"
        else:
        ...
        automation_label = _shorten(_automation_status_text(registry_workers), limit=96)
        if snapshot.registry_error:
            automation_label = _shorten(f"registry: {snapshot.registry_error}", limit=96)
        ...
        details = discord.ui.Button(
            ...
            disabled=not bool(self._selected_worker() or self._selected_is_legacy() or self._selected_is_auto()),
        )
        details.callback = self._show_worker_details
        ...
        if worker_select is not None:
            components.extend([
                discord.ui.Separator(),
                discord.ui.ActionRow(worker_select),
            ])

        components.append(discord.ui.ActionRow(refresh, details, pairing))
        secondary_buttons: list[Any] = []
        if not _snapshot_has_online_worker(snapshot):
            secondary_buttons.append(wake)
        if queued or running:
            secondary_buttons.append(cleanup_jobs)
        if secondary_buttons:
            components.append(discord.ui.ActionRow(*secondary_buttons))
    ...
    def _selected_worker_lines(self, snapshot: WorkerSnapshot) -> list[str]:
        ...
        if self._selected_is_auto():
            ...
            lines.append("⚙️ **Melhor worker disponível**")
            lines.append(f"-# {len(online_workers)} online · melhor agora: {best_name}")
        elif worker:
            ...
            device = _worker_device_label(worker)
            profile = _worker_profile_label(worker)
            lines.append(f"{icon} **{name}**")
            stale_note = _worker_stale_note(worker)
            if stale_note:
                lines.append(f"-# {stale_note}")
            middle = f"{ready} · visto {seen} · {profile} · v{version}"
            if device:
                middle += f" · {device}"
            lines.append(f"-# {middle}")
            health_line = f"{_battery_text(worker)} · {_simple_network_text(worker)}"
            queue_text = _queue_status_text(worker)
            if queue_text:
                health_line += f" · fila: {queue_text}"
            ...
            lines.append(f"🟢 **{_shorten(snapshot.name or 'Core Worker direto', limit=36)}**")
            status = snapshot.status if isinstance(snapshot.status, dict) else {}
            lines.append(f"-# direto online · v{version} · {_script_health_label({'status': status})}")
        elif workers:
        ...
        if snapshot.action_note:
            note = _shorten(_redact(snapshot.action_note), limit=96)
    ...
    async def _open_pairing_modal(self, interaction: discord.Interaction):
        ...
        class PairWorkerModal(discord.ui.Modal, title="Adicionar worker"):
    ...
    async def _show_onboarding_guide(self, interaction: discord.Interaction):
        ...
        msg = (
            ...
            "**Antes de começar no worker novo:**\n"
            "1. Instale o **Termux**.\n"
            "2. Conecte o **Tailscale** na mesma rede da VPS.\n"
            "3. Tenha a pasta `~/phone-worker` no Termux.\n\n"
            "**Depois, neste painel:**\n"
            "1. Escolha a categoria **Adicionar worker**.\n"
            "2. Use **Gerar código**.\n"
            "3. Copie o comando pronto e cole no Termux do worker novo.\n"
            "4. Quando terminar, toque **Atualizar**.\n\n"
            f"VPS detectada: `{base_url}`\n"
            "Perfis: `leve` (economia), `midia`/Normal (recomendado), `completo` (tarefas extras), `builder` (compila APK no Core Worker), `turbo` (máximo desempenho), `bedrock` (futuro)."
        )
    ...
    async def _show_worker_details(self, interaction: discord.Interaction):
        worker = self._selected_worker()
        if not worker:
            await interaction.response.send_message("Selecione um worker registrado para ver detalhes.", ephemeral=True)
            return
    ...
    async def _open_roles_modal(self, interaction: discord.Interaction):
        worker = self._selected_worker()
        if not worker:
            await interaction.response.send_message("Selecione um worker registrado para editar funções.", ephemeral=True)
            return
