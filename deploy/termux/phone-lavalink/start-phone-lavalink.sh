...
log "Lavalink offline. Reiniciando sessão $SESSION..."
tmux kill-session -t "$SESSION" 2>/dev/null || true
pkill -f 'java.*Lavalink.jar' 2>/dev/null || true

tmux new-session -d -s "$SESSION" \
  "proot-distro login '$DISTRO' -- bash -lc 'cd \"$PROOT_LAVALINK_DIR\" && exec java -Xmx$JAVA_XMX -jar Lavalink.jar'"
...
log "Falha ao iniciar Lavalink. Veja: tmux attach -t $SESSION"
exit 1
