    ...
    async def _ensure_connected(self, guild: discord.Guild, voice_channel) -> Optional[discord.VoiceClient]:
        ...
        async with lock:
            ...
            async def _ensure_self_deaf() -> None:
                try:
                    me = guild.me
                    if me and me.voice and not me.voice.self_deaf:
                        await guild.change_voice_state(channel=voice_channel, self_deaf=True)
                        print(f"[tts_voice] Self-deaf aplicado no canal {voice_channel.id} na guild {guild.id}")
                except Exception as deaf_err:
                    print(f"[tts_voice] Falha ao aplicar self-deaf na guild {guild.id}: {deaf_err}")
