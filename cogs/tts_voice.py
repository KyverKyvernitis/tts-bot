...
def _attachment_tts_label(attachment: discord.Attachment) -> str:
    ...
    if filename.endswith('.gif'):
        return "gif"
    if any(filename.endswith(ext) for ext in ('.png', '.jpg', '.jpeg', '.webp', '.bmp', '.svg')):
        return "anexo de imagem"
    if any(filename.endswith(ext) for ext in ('.mp4', '.mov', '.webm', '.mkv', '.avi', '.m4v')):
        return "anexo de vídeo"
    return "anexo"


def _link_tts_label(url: str) -> str:
    parsed = urlparse(url)
    ...
    if not display_host:
        return 'link'
    return f"link de {display_host}, https/{display_host}"
