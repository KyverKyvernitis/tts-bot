...
class TTSVoice(TTSAudioMixin, commands.GroupCog, group_name="tts", group_description="Comandos de texto para fala"):
    ...
    async def _invalidate_prefix_panel(self, guild_id: int, user_id: int, panel_kind: str):
        key = self._prefix_panel_key(guild_id, user_id, panel_kind)
        payload = self._active_prefix_panels.pop(key, None)
        if not payload:
            return

        message, view = payload
        try:
            ...
            embed = discord.Embed(
                ...
                description="Esse painel foi invalidado porque um painel mais novo desse tipo foi aberto",
                color=discord.Color.red(),
            )
            embed.set_footer(text="Use o comando novamente para abrir o painel atual")
    ...
    async def _check_prefix_panel_cooldown(self, message: discord.Message, panel_kind: str) -> bool:
        ...
        stale = [k for k, ts in self._prefix_panel_cooldowns.items() if ts < now - 30.0]
    ...
    async def _send_prefix_panel(
        ...
        *,
        ...
        await self._invalidate_prefix_panel(message.guild.id, message.author.id, panel_kind)

        sent = await message.channel.send(embed=embed, view=view)
        view.message = sent
        self._active_prefix_panels[self._prefix_panel_key(message.guild.id, message.author.id, panel_kind)] = (sent, view)
    ...
    async def _apply_mode_from_panel(self, interaction: discord.Interaction, mode: str, *, server: bool):
        ...
        view = self._build_panel_view(interaction.user.id, interaction.guild.id, server=server)
