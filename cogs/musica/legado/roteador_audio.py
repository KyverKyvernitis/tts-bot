...
MUSIC_SEARCH_RESULTS = max(1, min(10, int(getattr(config, "MUSIC_SEARCH_RESULTS", 3))))
...
class AudioRouter:
    ...
    async def play_tts_via_music_agent(
        ...
        *,
        ...
        audio_b64: str = "",
        audio_format: str = "",
        prebuilt_audio: bool = False,
        prebuilt_audio_source: str = "",
        tld: str = "com",
        tts_request_id: str = "",
    ) -> dict[str, Any]:
        ...
        result = await _music_agent_command(
            ...
            prebuilt_audio=bool(prebuilt_audio),
            prebuilt_audio_source=str(prebuilt_audio_source or ""),
            tld=tld,
    ...
    async def request_shuffle(self, guild_id: int, member) -> tuple[bool, str]:
        ...
        if usar_controles_fila_remota(self, state):
            remote = await atualizar_estado_controle_remoto(self, guild_id, create_panel=False)
            if remote:
                state = self.get_state(guild_id)
        ...
        if max(remote_count, local_count) <= 1 and not usar_controles_fila_remota(self, state):
