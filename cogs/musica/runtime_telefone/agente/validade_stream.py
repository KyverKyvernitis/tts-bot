...
def normalize_reference(raw: Any, guild_id: int) -> dict[str, int]:
    if not isinstance(raw, dict):
        raise DiscordAttachmentError("Referência do vídeo inválida.")
    try:
        ref = {key: int(raw[key]) for key in _REF_FIELDS}
    except (KeyError, ValueError, TypeError):
        raise DiscordAttachmentError("Referência do vídeo inválida.") from None
    if any(value <= 0 for value in ref.values()) or ref["guild_id"] != guild_id:
        raise DiscordAttachmentError("O vídeo não pertence a este servidor.")
    return ref
...
def valid_cdn_url(raw: Any) -> str:
    ...
    try:
        ...
        raise DiscordAttachmentError("O Discord não retornou uma URL válida para este vídeo.") from None
    return url
...
async def fetch_discord_attachment(client: Any, reference: dict[str, int]) -> dict[str, Any]:
    ...
    try:
        ...
        raise DiscordAttachmentError("Não consegui acessar o vídeo no Discord. Confira as permissões de leitura da mensagem.") from None
    if not isinstance(raw, dict) or str(raw.get("guild_id") or reference["guild_id"]) != str(reference["guild_id"]):
        raise DiscordAttachmentError("A mensagem do vídeo não corresponde ao servidor.")
    for attachment in raw.get("attachments") or ():
        ...
        if not (mime.startswith("video/") or (mime in _GENERIC_MIME and ext in _VIDEO_EXTENSIONS)):
            raise DiscordAttachmentError("Esse anexo não é um vídeo reproduzível.")
        item = dict(attachment)
        item["url"] = valid_cdn_url(item.get("url"))
        return item
    raise DiscordAttachmentError("O vídeo foi removido ou não está mais acessível.")
...
async def probe_discord_audio(url: str, *, executable: str = "ffprobe", timeout: float = 10.0) -> dict[str, Any]:
    ...
    try:
        ...
        raise DiscordAttachmentError("Não consegui iniciar a verificação de áudio do vídeo.") from None
    try:
        stdout, _ = await asyncio.wait_for(proc.communicate(), timeout=max(1.0, float(timeout)))
    except asyncio.TimeoutError:
        proc.kill()
        await proc.communicate()
        raise DiscordAttachmentError("A verificação do áudio demorou demais. O vídeo não entrou na fila.") from None
    ...
    if proc.returncode != 0 or len(stdout) > 65536:
        raise DiscordAttachmentError("Não consegui ler o áudio desse vídeo. Ele não entrou na fila.")
    try:
    ...
    if not isinstance(audio, dict):
        raise DiscordAttachmentError("Este vídeo não contém uma faixa de áudio reproduzível.")
    duration = _positive_duration(audio.get("duration")) or _positive_duration((payload.get("format") or {}).get("duration"))
    if duration is None:
        raise DiscordAttachmentError("Não consegui confirmar a duração do áudio. O vídeo não entrou na fila.")
    try:
        stream_index = int(audio["index"])
        if stream_index < 0:
            raise ValueError
    except (ValueError, KeyError, TypeError):
        raise DiscordAttachmentError("Não consegui identificar a faixa de áudio do vídeo.") from None
    try:
