...
def schedule_playlist_refill_if_needed(router: Any, guild_id: int, remote: dict[str, Any]) -> bool:
    ...
    Esta função é chamada pelo monitor que já consulta o Music Agent. Assim o
    lazy loading reaproveita o mesmo snapshot de estado e não adiciona requests
    periódicos só para playlists.
    """
    ...
    async def runner() -> None:
        try:
            ...
            batch = None
            # Uma repetição curta cobre falha transitória do embed sem manter um
            # loop próprio. O monitor continua sendo a única cadência contínua.
            for attempt in range(2):
                ...
                try:
                    ...
                    raise
                except Exception as exc:  # pragma: no cover - retry path asserted indirectly
                    last_exc = exc
                    if attempt == 0:
                        await asyncio.sleep(0.8)
            if last_exc is not None:
                logger.warning(
                    "[music/playlist] refill público falhou | guild=%s provider=%s offset=%s erro=%s",
                    guild_id,
                    cursor.provider,
                    cursor.next_offset,
                    last_exc,
            ...
            if remote_state and router.current_music_operation_generation(guild_id) == generation:
            ...
            logger.info(
                "[music/playlist] janela adicionada | guild=%s added=%s offset=%s next=%s exhausted=%s",
                guild_id,
                len(batch.tracks),
                cursor.next_offset,
            ...
            logger.debug("[music/playlist] refill encerrou com erro | guild=%s", guild_id, exc_info=True)
        finally:
