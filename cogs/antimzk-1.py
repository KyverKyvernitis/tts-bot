...
class AntiMzkCog(commands.Cog):
    ...
    def _member_has_ignored_tts_role(self, member: discord.Member | None) -> bool:
        if member is None or member.guild is None:
            return False

        ignored_tts_role_id = 0
        try:
            ignored_tts_role_id = max(0, int(self.db.get_ignored_tts_role_id(member.guild.id) or 0))
        except Exception:
            ignored_tts_role_id = 0

        if ignored_tts_role_id <= 0:
            return False

        return any(int(getattr(role, "id", 0) or 0) == ignored_tts_role_id for role in getattr(member, "roles", []))

    def _target_suffix_for_member(self, member: discord.Member | None) -> str:
        if member is None:
            return ""
    ...
    def get_target_suffix_for_message(self, message: discord.Message, member: discord.Member | None) -> str:
        ...
        if not any(getattr(target, "id", None) == member.id for target in targets):
            return ""

        return self._target_suffix_for_member(member)
