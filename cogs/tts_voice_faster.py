...
class TTSVoice(TTSAudioMixin, commands.GroupCog, group_name="tts", group_description="Comandos de texto para fala"):
    ...
    async def _warm_connect(self, guild: discord.Guild, voice_channel) -> None:
        if voice_channel is None:
            return

        state = self._get_state(guild.id)
        current_task = getattr(state, "connect_task", None)
        ...
        async def runner():
            try:
                ...
                self._tts_debug(f"[tts_voice] warm connect falhou | guild={guild.id} channel={getattr(voice_channel, 'id', None)} erro={e}")
    ...
    async def on_message(self, message: discord.Message):
        ...
        print(
            ...
            f"canal_voz={voice_channel.id} engine={resolved['engine']} forced_gtts={forced_gtts}"
        )
