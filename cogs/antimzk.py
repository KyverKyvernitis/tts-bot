...
_TARGET_NICK_SUFFIX_MUTED = " [censurado]"
_TARGET_NICK_SUFFIX_IGNORED = " [antitts]"
_TARGET_NICK_SUFFIX_BOTH = " [ultra-censurado]"
_TARGET_NICK_KNOWN_SUFFIXES = (
    _TARGET_NICK_SUFFIX_BOTH,
    _TARGET_NICK_SUFFIX_MUTED,
    _TARGET_NICK_SUFFIX_IGNORED,
)
...
class AntiMzkCog(commands.Cog):
    ...
    def _get_ignored_tts_role_id(self, guild_id: int) -> int:
        try:
            return max(0, int(self.db.get_ignored_tts_role_id(guild_id) or 0))
        except Exception:
            return 0

    def _member_has_ignored_tts_role(self, member: discord.Member) -> bool:
        ignored_role_id = self._get_ignored_tts_role_id(member.guild.id)
        if ignored_role_id <= 0:
            return False
        return any(int(getattr(role, "id", 0) or 0) == ignored_role_id for role in getattr(member, "roles", []))

    def _target_nick_suffix(self, member: discord.Member) -> str:
        is_muted = False
        ...
        ignores_tts = self._member_has_ignored_tts_role(member)

        if is_muted and ignores_tts:
            return _TARGET_NICK_SUFFIX_BOTH
        if is_muted:
            return _TARGET_NICK_SUFFIX_MUTED
        if ignores_tts:
            return _TARGET_NICK_SUFFIX_IGNORED
        return ""

    def _strip_target_nick_suffix(self, value: str | None) -> str:
        text = str(value or "")
        changed = True
        while changed:
            changed = False
            for suffix in _TARGET_NICK_KNOWN_SUFFIXES:
                if text.endswith(suffix):
                    text = text[:-len(suffix)].rstrip()
                    changed = True
        return text

    async def _sync_target_nickname(self, member: discord.Member):
        base_nick = getattr(member, "nick", None)
        base_name = self._strip_target_nick_suffix(base_nick if base_nick is not None else getattr(member, "display_name", None) or getattr(member, "name", None) or "")
        suffix = self._target_nick_suffix(member)

        if base_nick is None:
            desired_nick = None if not suffix else f"{base_name or getattr(member, 'name', 'usuário')}{suffix}"
        else:
            desired_nick = f"{base_name}{suffix}" if suffix else (base_name or None)

        if desired_nick is not None and len(desired_nick) > 32:
            trim_to = max(0, 32 - len(suffix))
            trimmed_base = (base_name or getattr(member, 'name', ''))[:trim_to].rstrip()
            desired_nick = f"{trimmed_base}{suffix}" if suffix else (trimmed_base or None)

        current_nick = getattr(member, "nick", None)
        if desired_nick == current_nick:
            return

        try:
            await member.edit(nick=desired_nick, reason="modo censura sync target suffix")
        except Exception:
            pass

    async def _sync_targets_nicknames(self, targets: list[discord.Member]):
        for target in targets:
            await self._sync_target_nickname(target)
    ...
    async def _handle_role_toggle_trigger(self, message: discord.Message) -> bool:
        ...
        ignored_tts_role_id = self._get_ignored_tts_role_id(guild.id)
        ...
        await self._sync_targets_nicknames(targets)
