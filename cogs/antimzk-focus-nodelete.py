...
class AntiMzkCog(commands.Cog):
    ...
    _ANTI_MZK_SUFFIXES = (" [ultra-censurado]", " [censurado]", " [bot ignora]")
    ...
    async def _refresh_target_suffix_nickname(self, member: discord.Member, ignored_tts_role: discord.Role | None):
        ...
        current_name = current_nick if current_nick is not None else member.name
        base_name = self._strip_antimzk_suffix(current_name)
        suffix = self._target_suffix(member, ignored_tts_role)
        desired_full = f"{base_name}{suffix}".strip()

        if current_nick is None:
            if desired_full == member.name:
                return
            if not suffix:
                return
            new_nick = desired_full
        else:
            if not suffix and base_name == member.name:
                new_nick = None
            else:
                new_nick = desired_full
