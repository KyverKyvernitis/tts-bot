...
class TTSVoice(TTSAudioMixin, commands.GroupCog, group_name="tts", group_description="Comandos de texto para fala"):
    ...
    def _modo_censura_target_suffix(self, message: discord.Message, member: discord.Member | None) -> str:
        if member is None:
            return ""

        anti_mzk_cog = self.bot.get_cog("AntiMzkCog")
        if anti_mzk_cog is None:
            return ""

        getter = getattr(anti_mzk_cog, "get_target_suffix_for_message", None)
        if getter is None:
            return ""

        try:
            return str(getter(message, member) or "")
        except Exception:
            return ""
    ...
    def _tts_user_reference(self, member: discord.abc.User | None, *, guild_id: int | None = None, message: discord.Message | None = None) -> str:
        spoken, _ = self._resolve_spoken_name(member, guild_id=guild_id)
        if message is not None and isinstance(member, discord.Member):
            spoken = f"{spoken}{self._modo_censura_target_suffix(message, member)}"
        return spoken
    ...
    def _render_tts_text(self, message: discord.Message, raw_text: str) -> str:
        ...
        def replace_user(match: re.Match[str]) -> str:
            ...
            return self._tts_user_reference(member, guild_id=getattr(message.guild, "id", None), message=message)
