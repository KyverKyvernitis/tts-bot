...
from ..mensagens.referencias import descricoes_anexos_tts


def anexar_descricoes_tts(text: str, descriptions: list[str], *, normalize_spaces: Callable[[str], str]) -> str:
...
def renderizar_texto_tts_mensagem(
    message,
    raw_text: str,
    *,
    guild_id: int | None,
    ...
    if not has_mentions_or_links:
        ...
        if not attachments:
            return text
        descriptions = descricoes_anexos_tts(
            attachments,
            extensoes_imagem=image_extensions,
            extensoes_video=video_extensions,
        )
        return anexar_descricoes_tts(text, descriptions, normalize_spaces=normalize_spaces)
