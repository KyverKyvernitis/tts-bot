...
def test_entrypoint_e_infra_legados_foram_removidos():
    assert CANONICO.is_file()
    assert "load_pending_local_candidate()" in ler_fonte_core()
    for rel in (
        "scripts/tts-bot-update.sh",
        "deploy/scripts/tts-bot-update.sh",
...
def test_utilitarios_canonicos_sem_fachadas_legadas():
    ...
    for legado, canonico in pares.items():
        assert (ROOT / "updater" / "utilitarios" / canonico).is_file()
        assert not (ROOT / "utility" / legado).exists()
