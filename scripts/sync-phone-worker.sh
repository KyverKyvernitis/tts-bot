...
for f in start-phone-worker.sh watch-phone-worker.sh install.sh README.md phone-worker.env.example; do
  if [ -f "$SRC_DIR/$f" ]; then
    case "$f" in
      start-phone-worker.sh|watch-phone-worker.sh)
...
"${SSH_BASE[@]}" "
chmod +x '$REMOTE_HOME/start-phone-worker.sh' '$REMOTE_HOME/watch-phone-worker.sh' 2>/dev/null || true
