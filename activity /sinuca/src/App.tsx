...
function sectionConfiguredLabel(section: DashboardSectionSummary | undefined) {
  if (!section) return "";
  const enabled = section.enabled === null ? "" : section.enabled ? " · ativo" : " · desativado";
...
export default function App() {
  ...
  const [message, setMessage] = useState<string>("Abrindo Activity Dashboard...");
  ...
  async function login(prompt: "none" | "consent" = "consent") {
    ...
    try {
      ...
      const exchanged = await exchangeDiscordTokenRequest(auth.code);
      if (!exchanged.ok || !exchanged.accessToken) {
        setAuthState("error");
        setMessage(`Não consegui trocar autorização por token: ${exchanged.error || exchanged.detail || "erro desconhecido"}`);
        return;
      }
  ...
  async function saveSection() {
    ...
    for (const field of selectedSection.fields) {
      if (draft[field.id] !== values[field.id]) {
        updates[field.id] = draft[field.id];
      }
    }
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : "Não consegui salvar as alterações.");
    } finally {
      setSaving(false);
    }
  }
  ...
  useEffect(() => {
    ...
    bootstrapDiscord().then((result) => {
      ...
      if (cancelled) return;
      setAuthState("error");
      setMessage(error instanceof Error ? error.message : "Falha ao inicializar o SDK do Discord.");
    });
  ...
  const hasUnsaved = selectedSection?.fields.some((field) => draft[field.id] !== values[field.id]) ?? false;
    ...
    <main className="dashboard-shell">
      <section className="dashboard-hero">
        <div>
          ...
          <p className="hero-text">Central administrativa interativa para configurar o bot neste servidor.</p>
      ...
      {authState === "needs_login" && (
        <section className="auth-card">
          ...
          <p>O dashboard usa sua conta Discord para validar se você pode configurar este servidor.</p>
          <button className="primary-button" disabled={busy} onClick={() => void login("consent")}>Autorizar</button>
        </section>
      )}
        ...
        <section className="dashboard-grid">
          <aside className="section-list" aria-label="Áreas configuráveis">
            ...
            {summary.map((item) => (
              <button
                key={item.id}
                className={`section-button ${selectedSection?.id === item.id ? "section-button--active" : ""}`}
              ...
              <>
                <div className="settings-heading">
                  ...
                  <div className="button-row">
                    ...
                    <button className="primary-button" onClick={() => void saveSection()} disabled={!hasUnsaved || saving}>{saving ? "Salvando..." : "Salvar seção"}</button>
                  </div>
                </div>
                      ...
                      <label key={field.id} className={`field-card ${changed ? "field-card--changed" : ""}`}>
                        ...
                        {field.type === "boolean" ? (
                          ...
                          <input
                            ...
                            maxLength={field.maxLength}
                            value={stringifyValue(current)}
                            placeholder={field.placeholder ?? (field.type === "channel" ? "ID ou menção do canal" : field.type === "role" ? "ID ou menção do cargo" : "")}
                            onChange={(event) => updateDraft(field, event.target.value)}
                          />
                        )}
