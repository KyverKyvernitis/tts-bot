...
def test_alert_script_emits_durable_bot_job_instead_of_webhook_payload() -> None:
    ...
    assert payload["delivery"] == "discord_bot"
    assert payload["type"] == "success"
    assert payload["title"] == "Atualização concluída"
    assert payload["body"] == body
    assert payload["attempts"] == 0
    assert payload["event_id"].startswith("log-")


def test_alert_script_has_no_webhook_delivery_path() -> None:
    source = (ROOT / "alert.sh").read_text(encoding="utf-8")
    assert "curl" not in source
    assert "urllib" not in source
    assert "ALERT_WEBHOOK_URL" not in source
    assert '"delivery": "discord_bot"' in source
    assert "UPDATE_LOG_OUTBOX_DIR" in source
...
def test_update_progress_keeps_detail_plain_and_exports_completed_duration_for_stage_line() -> None:
    harness = r'''
source <(awk '/^human_duration\(\)/{flag=1} /^mark_update_timing\(\)/{flag=0} flag' updater/core/atualizar.sh)
source <(awk '/^zip_progress_title\(\)/{flag=1} /^rollback_request_roots\(\)/{flag=0} flag' updater/core/atualizar.sh)
...
'''
    ...
    assert "✅ Base conferida" not in text
    assert '"completed_stage": "Base conferida"' in text
    assert '"completed_duration":' in text
...
def test_final_status_does_not_render_bare_ok_line() -> None:
    harness = r'''
source <(awk '/^build_final_status_description\(\)/{flag=1} /^deploy_bot\(\)/{flag=0} flag' updater/core/atualizar.sh)
...
def test_frontend_deploy_reports_dependency_build_publish_and_validation_stages() -> None:
    source = (ROOT / "updater" / "core" / "atualizar.sh").read_text(encoding="utf-8")
...
def test_update_progress_titles_follow_each_phase_without_naming_the_subsystem() -> None:
    harness = r'''
source <(awk '/^zip_progress_title\(\)/{flag=1} /^zip_progress_status\(\)/{flag=0} flag' updater/core/atualizar.sh)
ROLLBACK_CONTROL_MODE=0
...
def test_candidate_dirty_check_uses_ubuntu_git_and_fails_closed(tmp_path: Path) -> None:
    ...
    harness = r'''
source <(awk '/^candidate_local_changes_are_expected\(\)/{flag=1} /^ensure_no_unstaged_tracked_changes\(\)/{flag=0} flag' updater/core/atualizar.sh)
LOCAL_CANDIDATE_MODE=1
...
def test_systemd_overlay_uses_the_template_path_that_changed(tmp_path: Path) -> None:
    ...
    harness = r'''
source <(awk '/^managed_systemd_template_source\(\)/{flag=1} /^deploy_vps_systemd_units\(\)/{flag=0} flag' updater/core/atualizar.sh)
REPO_DIR="$TEST_REPO"
...
'''
    ...
    text = result.stdout
    assert "-# Pacote inspecionado" in text
    assert "-# Candidato seguro preparado" in text
    assert "✅ Pacote inspecionado" not in text
    assert "<a:loading:test> **Conferindo ZIP**" in text
