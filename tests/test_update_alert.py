...
def test_update_progress_keeps_detail_plain_and_exports_completed_duration_for_stage_line() -> None:
    harness = r'''
source <(awk '/^human_duration\(\)/{flag=1} /^mark_update_timing\(\)/{flag=0} flag' "$UPDATER_TEST_SOURCE")
source <(awk '/^zip_progress_title\(\)/{flag=1} /^rollback_request_roots\(\)/{flag=0} flag' "$UPDATER_TEST_SOURCE")
...
def test_final_status_does_not_render_bare_ok_line() -> None:
    harness = r'''
source <(awk '/^build_final_status_description\(\)/{flag=1} /^deploy_bot\(\)/{flag=0} flag' "$UPDATER_TEST_SOURCE")
...
def test_frontend_deploy_reports_dependency_build_publish_and_validation_stages() -> None:
    source = caminho_fonte_core().read_text(encoding="utf-8")
...
def test_update_progress_titles_follow_each_phase_without_naming_the_subsystem() -> None:
    harness = r'''
source <(awk '/^zip_progress_title\(\)/{flag=1} /^zip_progress_status\(\)/{flag=0} flag' "$UPDATER_TEST_SOURCE")
ROLLBACK_CONTROL_MODE=0
...
def test_candidate_dirty_check_uses_ubuntu_git_and_fails_closed(tmp_path: Path) -> None:
    ...
    harness = r'''
source <(awk '/^candidate_local_changes_are_expected\(\)/{flag=1} /^ensure_no_unstaged_tracked_changes\(\)/{flag=0} flag' "$UPDATER_TEST_SOURCE")
LOCAL_CANDIDATE_MODE=1
...
def test_systemd_overlay_uses_the_template_path_that_changed(tmp_path: Path) -> None:
    ...
    harness = r'''
source <(awk '/^managed_systemd_template_source\(\)/{flag=1} /^deploy_vps_systemd_units\(\)/{flag=0} flag' "$UPDATER_TEST_SOURCE")
REPO_DIR="$TEST_REPO"
