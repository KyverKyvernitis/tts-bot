...
def test_partial_snapshot_failure_does_not_prevent_heartbeat_payload(control, monkeypatch, failure):
    probe = "_system_status"
    def fail():
        if failure == "raises":
...
def test_control_module_does_not_mutate_inputs(control):
    ...
    snapshots = {"system": {"ffprobe": True}, "battery": {"level": 7},
                 "network": {"type": "wifi"}, "updater": {"state": "idle"}}
    ...
    assert "ffprobe" in result["capabilities"]
